...
from models_fmnist_nn2 import FmnistNN2
...
class DummyArgs:
    def __init__(self, algorithm, num_users, alpha):
        self.dataset = f'fmnist_alpha{alpha}_n{num_users}'
        self.model = 'fmnist_nn2'
...
def load_fmnist_test_from_csv(path="fmnist_test.csv", batch_size=32):
        ...
        for algorithm in ["FedProx"]:
            for num_users in [2, 5, 10]:
                for alpha in ["balanced", "0.5", "0.1"]:
                    ...
                    test_loader = load_fmnist_test_from_csv(batch_size=args.batch_size)
