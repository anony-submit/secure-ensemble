...
def create_custom_data_loaders(distribution_path, num_users, batch_size=32):
    ...
    for user_id in range(num_users):
        ...
        for class_id, count in dist[user_id]:
            if count > len(class_indices[class_id]):
                raise ValueError(f"[ERROR] User {user_id}: Requested {count} samples from class {class_id}, but only {len(class_indices[class_id])} available.")
