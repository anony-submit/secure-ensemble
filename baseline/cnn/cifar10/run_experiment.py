...
from models_cifar10_nn3 import Cifar10NN3
...
class DummyArgs:
    def __init__(self, algorithm, num_users, alpha):
        ...
        self.model = 'cifar10_nn3'
        self.algorithm = algorithm
        self.batch_size = 32
        self.local_epochs = 32
        self.num_users = num_users
        self.learning_rate = 0.015
        self.num_glob_iters = 200
        self.device = get_preferred_device()
        self.embedding = 0
        self.result_path = './results'
        self.train = 3
        self.K = 10
        self.personal_learning_rate = 0.015 # balanced 0.02 
        self.times = 1
        self.beta = 1.0
        self.lamda = 1
...
def load_cifar10_test_from_csv(path="cifar10_test.csv", batch_size=32):
    df = pd.read_csv(path, header=0) 
    labels = torch.tensor(df["label"].values).long()
    images = torch.tensor(df.drop(columns=["label"]).values).float().reshape(-1, 3, 32, 32)
        ...
        for algorithm in ["FedAvg", "FedProx"]:
            for num_users in [2, 5, 10]:
                ...
                # for alpha in ["balanced", "0.5", "0.1"]:
                    ...
                    model = Cifar10NN3(), args.model
                    server = create_server(args, model, user_datasets)
                    test_loader = load_cifar10_test_from_csv(batch_size=args.batch_size)
