...
def main():
    ...
    df.to_csv('test_data/mnist_test.csv', index=False)
            ...
            for method_name, alpha in split_methods:
                ...
                for mode in training_modes:
                    ...
                    for i, subset in enumerate(subsets):
                        ...
                        print(f"NeuralNetwork {i+1} accuracy (full test): {accuracy:.2f}%")
                        print(f"NeuralNetwork {i+1} accuracy (fixed test): {fixed_accuracy:.2f}%")
                        
                        accuracies.append(accuracy)
                        fixed_accuracies.append(fixed_accuracy)
                        
                        net.load_state_dict({k: torch.tensor(v) for k, v in params.items()})
                        models.append(net)
