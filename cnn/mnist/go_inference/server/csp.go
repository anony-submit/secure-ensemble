...
type EncryptedMNISTModel struct {
...
type CSPServer struct {
	...
	timing          CSPTiming
	encryptedModels map[string]*EncryptedMNISTModel
...
func NewCSPServer(params mkckks.Parameters) *CSPServer {
	return &CSPServer{
		...
		encryptedModels:  make(map[string]*EncryptedMNISTModel),
...
func (s *CSPServer) EnrollModel(ctx context.Context, req *pb.EnrollModelRequest) (*pb.EnrollModelResponse, error) {
	...
	s.encryptedModels[req.OwnerId] = &EncryptedMNISTModel{
...
func (s *CSPServer) performInference(input *mkckks.Ciphertext, model *EncryptedMNISTModel) *mkckks.Ciphertext {
	layer1Output := s.fc1Layer(input, model.FC1Weights, model.FC1Bias)
...
func (s *CSPServer) encryptZero(id string) *mkckks.Ciphertext {
	...
	return s.encryptor.EncryptMsgNew(msg, s.pkSet.GetPublicKey(id))
}
...
func (s *CSPServer) Close() error {
	...
	s.encryptedModels = make(map[string]*EncryptedMNISTModel)
