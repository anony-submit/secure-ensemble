...
func (d *DataOwner) EnrollModel(fc1w [][]float64, fc1b []float64, fc2w [][]float64, fc2b []float64) error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *DataOwner) PerformPartialDecryption(ctx context.Context, req *pb.PartialDecryptionRequest) (*pb.PartialDecryptionResponse, error) {
	...
	d.timing.PartialDecryptionTime = time.Duration(time.Now().UnixNano() - startTime)
