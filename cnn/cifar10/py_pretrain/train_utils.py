...
def train_model(net, trainloader, testloader, fixed_testloader, device, 
    ...
    elif training_mode.startswith('balsoftmax_entropy_'):
        entropy_weight = float(training_mode.split('_')[-1]) / 10
        criterion = BalancedSoftmaxLossWithEntropyReg(10, sample_per_class, entropy_weight)
    else:
        raise ValueError(f"Unsupported training mode: {training_mode}")

    optimizer = optim.AdamW(net.parameters(), lr=0.001, weight_decay=0.01)
    epochs = 20 if isinstance(net, ResNet18().__class__) else 100
