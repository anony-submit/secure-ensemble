...
import (
	"cnn/cifar10/go_inference/client"
	"cnn/cifar10/go_inference/common"
	"cnn/cifar10/go_inference/dataowner"
	"cnn/cifar10/go_inference/server"
	"fmt"
	"math"
	"math/rand/v2"
	"os"
...
func TestCIFAR10Dropout(t *testing.T) {
	if testing.Short() {
	...
	distributions := []string{"dirichlet0.1"}
...
func runDropoutExperiment(t *testing.T, config DropoutExperimentConfig) {
	...
	fmt.Printf("\n=== Starting CIFAR10 Dropout Experiment ===\n")
...
func writeDropoutResultsToFile(config DropoutExperimentConfig, results DropoutExperimentResult) error {
	...
	output := fmt.Sprintf(`
=== CIFAR10 Dropout Experiment Results ===
