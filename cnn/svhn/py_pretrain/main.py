...
def main():
    ...
    test_labels = np.array([testset.labels[i] for i in fixed_indices])
    ...
    df.to_csv('test_data/svhn_test.csv', index=False)
    
    base_dir = 'results'
    experiment_dir = os.path.join(base_dir, f'svhn_experiment_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
            ...
            for method_name, alpha in split_methods:
                ...
                if alpha is None:
                    ...
                    for i, subset in enumerate(subsets):
                        distribution[i] = np.bincount([trainset.labels[subset.indices[j]] for j in range(len(subset))], minlength=10)
                ...
                ensemble_acc = accuracy_score(testset.labels, ensemble_preds) * 100
                ensemble_fixed_acc = accuracy_score(
                    [testset.labels[i] for i in fixed_indices], ensemble_fixed_preds) * 100
                    ...
                    for i, subset in enumerate(subsets):
                        ...
                        if loss_type in ['balsoftmax', 'balsoftmax_entropy_5']:
                            trainloader = torch.utils.data.DataLoader(subset, batch_size=batch_size,
                                                                  shuffle=True, num_workers=8)
                            if alpha is not None:
                                sample_per_class = np.bincount([trainset.labels[subset.indices[j]] for j in range(len(subset))], minlength=10)
                        else:
                            randaug_trainset = augment_subset(subset, augmentation_type='randaug', n=2, apply_augment=True)
                            trainloader = torch.utils.data.DataLoader(randaug_trainset, batch_size=batch_size,
                                                                  shuffle=True, num_workers=8)
